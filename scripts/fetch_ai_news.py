    ...
    try:
            ...
            if response.status_code == 200:
                ...
                if data['code'] == 200:
                    for news in data['newslist']:
                        if any(keyword in news['title'] or keyword in news['description'] 
                               for keyword in ['AI', '人工智能', '大模型', 'GPT', 'Claude', '深度学习']):
                            news_list.append({
                                'id': hashlib.md5(news['url'].encode()).hexdigest(),
                                'title': news['title'],
                                'description': news['description'],
                                'author': news['source'],
                                'date': datetime.now().strftime('%Y-%m-%d'),
                                'image': news['picUrl'],
                                'category': get_category(news['title']),
                                'link': news['url']
                            })
        ...
        if response.status_code == 200:
            ...
            for item in data['data']['items']:
                if any(keyword in item['title'] for keyword in ['AI', '人工智能', '大模型', 'GPT', 'Claude', '深度学习']):
                    news_list.append({
                        'id': hashlib.md5(item['route'].encode()).hexdigest(),
                        'title': item['title'],
                        'description': item['description'] if 'description' in item else item['summary'],
                        'author': '36氪',
                        'date': datetime.now().strftime('%Y-%m-%d'),
                        'image': item['cover'] if 'cover' in item else '',
                        'category': get_category(item['title']),
                        'link': f"https://36kr.com{item['route']}"
                    })
    ...
    try:
        for source in RSS_SOURCES:
            ...
            for entry in feed.entries[:10]:  # 获取每个源的前10条新闻
                ...
                # 确保有相关的AI关键词
                if not any(keyword in title.lower() 
                          for keyword in ['ai', 'artificial intelligence', '人工智能', '机器学习', 
                                         'machine learning', 'deep learning', '深度学习', 
                                         'llm', 'gpt', 'claude', '大模型', '大语言模型']):
                    continue
                ...
                news_list.append({
                    ...
                    'date': date,
                    'image': image_url,
                    'category': get_rss_category(title, source['category']),
                    'link': entry.link
                })
...
def get_default_image_for_category(category):
    """根据类别返回默认图片"""
    default_images = {
        'AIGC': 'https://www.ihuayaoming.com/upload/20213/9c3a2e7e2a7c47fc88e76fbed41c5dab.jpg',
        '大模型': 'https://pic.rmb.bdstatic.com/bjh/news/1d9599ae7559e5427c0d2a3c36029c3a.jpeg',
        '科技': 'https://www.chinadaily.com.cn/china/images/attachement/jpg/site1/20170818/286ed488c10c1b12e40412.jpg',
        '硬件': 'https://www.nvidia.com/content/dam/en-zz/Solutions/about-nvidia/logo-and-brand/01-nvidia-logo-horiz-500x200-2c50-p@2x.png',
        'AI安全': 'https://img.zcool.cn/community/01de2f58bea187a801219c777fcf9f.jpg'
    }
    return default_images.get(category, 'https://static.xiaohucdn.com/blog/ai/default-news.jpg')

def get_rss_category(title, default_category):
    """根据标题和默认分类确定新闻类别"""
    if any(word in title for word in ['GPT', 'Claude', 'LLM', '大模型', '文心', '通义', '大语言模型']):
        return '大模型'
    elif any(word in title for word in ['芯片', 'GPU', '计算', 'NVIDIA', '英伟达', '华为', '服务器']):
        return '硬件'
...
def get_category(title):
    """根据标题内容判断新闻类别"""
    if any(word in title for word in ['GPT', 'Claude', 'LLM', '大模型', '文心', '通义']):
        return '大模型'
    elif any(word in title for word in ['芯片', 'GPU', '计算', 'NVIDIA', '英伟达', '华为']):
        return '硬件'
...
def update_news_ts(news_list):
    ...
    try:
        ...
        start_pattern = re.compile(r'export const defaultNewsItems: CardItem\[\] = \[')
        ...
        # 过滤掉已存在的新闻
        filtered_news = [news for news in news_list if news['id'] not in existing_ids]
