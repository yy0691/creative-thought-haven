...
import generated from '../data/ai/generated-ai-sections.json';
// FIX: Removed unused imports like axios, useMediaQuery, FaCube, FaMicrochip to clean up the code.
...
// --- FIX: Step 1 - Define Missing Types and Interfaces ---
// The CardItem type was used everywhere but not defined.
// Based on its usage, this interface covers all the properties your code needs.
interface CardItem {
  id: string;
  title: string;
  description: string;
...
// --- FIX: Step 2 - Create Placeholder Data and Components ---
// These are placeholders for data and components imported from other files.
// Replace these with your actual imports.

// Placeholder for CardItem data arrays
const toolsItems: CardItem[] = [{ id: '1', title: 'Example Tool', description: 'This is a great tool.', category: 'AI编程', link: '#', image: 'https://via.placeholder.com/300' }];
const promptsItems: CardItem[] = [{ id: '1', title: 'Example Prompt', description: 'A useful prompt.', category: 'Prompts', link: '#', image: 'https://via.placeholder.com/300' }];
const learningItems: CardItem[] = [];
const coursesItems: CardItem[] = [];
const deeplearningItems: CardItem[] = [];
const linksItems: CardItem[] = [{ id: '1', title: 'Awesome AI Link', description: 'A link to a great resource.', category: '学术资源', link: '#'}];
const tutorialsItems: CardItem[] = [];


// Placeholder for the useNews custom hook
const useNews = () => {
  const newsData: CardItem[] = [
    { id: 'news1', title: 'AI Breakthrough', description: 'A new AI model has been released.', category: 'News', link: '#', image: 'https://via.placeholder.com/300', date: '2025-09-27' }
  ];
...
// useAISections: wire to generated JSON so tabs map to content intelligently
const useAISections = () => {
    const aiSections = generated || [];
    return { aiSections };
};

// 动态分类匹配系统
const createDynamicTabCategories = (aiSections: any[]) => {
    // 基础分类结构
    const baseCategories = [
        {
            ...
            tabs: [
                ...
                { id: 'tutorials', label: 'AI教程', icon: 'BookOpen' },
                { id: 'prompts', label: '提示词库', icon: 'Terminal' },
            ],
        },
        {
            id: 'learning',
            title: '学习',
            tabs: [
                { id: 'learning', label: '学习路线', icon: 'Map' },
                { id: 'courses', label: '系列课程', icon: 'Youtube' },
                { id: 'deeplearning', label: '深度学习', icon: 'BrainCircuit' },
            ],
        },
        {
            id: 'resources',
            title: '资源',
            tabs: [
                { id: 'links', label: '资源链接', icon: 'Link' },
            ],
        },
    ];

    // 动态生成工具分类
    const toolsSection = aiSections.find(section => section.id === 'tools');
    const toolsTabs = [];
    
    if (toolsSection && toolsSection.items) {
        // 分析工具项目，动态生成子分类
        const categoryMap = new Map();
        
        toolsSection.items.forEach((item: any) => {
            const category = inferCategoryFromTitle(item.title, item.description);
        ...
        if (categoryMap.size > 0) {
            toolsTabs.push({ id: 'general', label: '通用大模型', icon: 'Bot' });
            
            // 检查是否有绘画相关工具
            if (Array.from(categoryMap.keys()).some(cat => 
                cat.includes('绘画') || cat.includes('图像') || cat.includes('艺术'))) {
                toolsTabs.push({ id: 'painting', label: 'AI绘画', icon: 'Paintbrush' });
            }
            
            // 检查是否有编程相关工具
            if (Array.from(categoryMap.keys()).some(cat => 
                cat.includes('编程') || cat.includes('代码') || cat.includes('开发'))) {
                toolsTabs.push({ id: 'coding', label: 'AI编程', icon: 'Code' });
            }
            
            // 检查是否有语音相关工具
            if (Array.from(categoryMap.keys()).some(cat => 
                cat.includes('语音') || cat.includes('音频') || cat.includes('声音'))) {
                toolsTabs.push({ id: 'voice', label: 'AI语音', icon: 'AudioLines' });
            }
            
            // 检查是否有视频相关工具
            if (Array.from(categoryMap.keys()).some(cat => 
                cat.includes('视频') || cat.includes('影像'))) {
                toolsTabs.push({ id: 'video', label: 'AI视频', icon: 'Video' });
            }
            
            // 检查是否有建模相关工具
            if (Array.from(categoryMap.keys()).some(cat => 
                cat.includes('建模') || cat.includes('3D'))) {
                toolsTabs.push({ id: 'modeling', label: 'AI建模', icon: 'Cube' });
            }
            
            // 检查是否有安全相关工具
            if (Array.from(categoryMap.keys()).some(cat => 
                cat.includes('安全') || cat.includes('防护'))) {
                toolsTabs.push({ id: 'security', label: 'AI安全', icon: 'Shield' });
            }
            
            // 检查是否有智能体相关工具
            if (Array.from(categoryMap.keys()).some(cat => 
                cat.includes('智能体') || cat.includes('Agent') || cat.includes('助手'))) {
                toolsTabs.push({ id: 'agent', label: 'AI智能体', icon: 'Users' });
            }
            
            // 其他分类
            toolsTabs.push({ id: 'other', label: '其他', icon: 'AppWindow' });
        }
    }

    // 如果没有动态生成的工具标签页，使用默认的
    if (toolsTabs.length === 0) {
        toolsTabs.push(
            { id: 'general', label: '通用大模型', icon: 'Bot' },
            { id: 'painting', label: 'AI绘画', icon: 'Paintbrush' },
            { id: 'coding', label: 'AI编程', icon: 'Code' },
            { id: 'voice', label: 'AI语音', icon: 'AudioLines' },
            { id: 'video', label: 'AI视频', icon: 'Video' },
            { id: 'modeling', label: 'AI建模', icon: 'Cube' },
            { id: 'security', label: 'AI安全', icon: 'Shield' },
            { id: 'agent', label: 'AI智能体', icon: 'Users' },
            { id: 'other', label: '其他', icon: 'AppWindow' }
        );
    }
    ...
    return [...baseCategories.slice(0, 2), toolsCategory, ...baseCategories.slice(2)];
};

// Placeholder for SmallToolCard component
const SmallToolCard = ({ item, onOpenDetail }: { item: CardItem, onOpenDetail: (item: CardItem) => void }) => (
  <div 
  ...
  >
    <img src={getFaviconUrl(item.link)} alt="" className="w-6 h-6 mb-2" />
...
const Loader = () => <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>;

// Placeholder for TabIcon component
...
// Placeholder for tab categories data.
...
// Placeholder for mapCategoryToTabId function
const mapCategoryToTabId = (category: string): string => {
  // Simple mapping logic, adjust as needed
  if (category.includes('绘画')) return 'painting';
  if (category.includes('编程')) return 'coding';
  return 'general';
};


// --- Auto favicon + category inference helpers ---
...
const inferCategoryFromTitle = (title: string, description: string = ''): string => {
  const text = `${title} ${description}`.toLowerCase();
  
  // AI工具分类推断
  if (/(chatgpt|claude|gemini|gpt|文心一言|豆包|通义千问|deepseek|kimi|对话|聊天|通用|大模型|llm)/.test(text)) return '通用大模型';
  if (/(midjourney|dall-e|stable diffusion|leonardo|comfyui|绘画|图像|艺术|画|生成图|art|paint|image|draw)/.test(text)) return 'AI绘画';
  if (/(github copilot|cursor|v0|codeium|编程|代码|开发|code|programming|dev)/.test(text)) return 'AI编程';
  if (/(runway|pika|sora|heygen|视频|video|影像|剪辑|生成视频)/.test(text)) return 'AI视频';
  if (/(openai tts|elevenlabs|suno|lovo|语音|音频|声音|音乐|tts|voice|audio|sound)/.test(text)) return 'AI语音';
  if (/(建模|3d|模型|modeling|blender)/.test(text)) return 'AI建模';
  if (/(安全|防护|security|protect)/.test(text)) return 'AI安全';
  if (/(智能体|agent|助手|assistant|bot)/.test(text)) return 'AI智能体';
  if (/(notion|gamma|tome|办公|文档|ppt|演示|office|document)/.test(text)) return 'AI办公';
  
  // 学习资源分类推断
  if (/(arxiv|paper|论文|学术|cite|publication)/.test(text)) return '学术资源';
  if (/(huggingface|github|gitlab|开源|repo|模型库|dataset)/.test(text)) return '开源平台';
  if (/(kaggle|竞赛|实战|实践|notebook|colab)/.test(text)) return '实践平台';
  if (/(趋势|trend|insight|热度|排行|指数)/.test(text)) return '趋势分析';
  if (/(google|deepmind|openai|microsoft|meta ai|研究|research|lab|institute)/.test(text)) return '研究机构';
...
const AI = () => {
  ...
  // FIX: Step 3 - Add missing state variables
  // These states were used in your functions but not declared.
  ...
  useEffect(() => {
    ...
    console.log('动态标签页分类:', tabCategories);
    
    // 新闻数据现在通过 Hook 自动加载，无需手动刷新
  }, [loadMarkdownContent, activeTab, tabCategories]); // 添加tabCategories作为依赖项
  ...
  const loadBlogPosts = async () => {
    try {
      ...
      // 筛选AI相关文章
      const aiRelatedPosts = allPosts.filter((post: any) => {
        // 检查类别
        ...
        // 检查标签
        const tagMatch = post.tags && post.tags.some((tag: string) => 
          ['AI', '人工智能', '机器学习', 'ML', '深度学习', 'LLM', 'GPT', 'ChatGPT', 'Stable Diffusion', 'Midjourney'].some(term => 
      ...
      // 将博客文章转换为CardItem格式
      const blogPostCards = aiRelatedPosts.map((post: any) => {
        // 确保slug正确编码处理
  ...
  const getTabContent = () => {
    try {
      switch(activeTab) {
        ...
        case 'tools': {
          const sec = (aiSections || []).find(s => s.id === 'tools');
          const fromSec = sec ? sec.items.map(it => ({ id: it.id, title: it.title, description: it.description || '', link: it.link || it.url || '#', image: (it.link || it.url) ? getFaviconUrl(it.link || it.url) : undefined, category: it.category || inferCategoryFromTitle(it.title || '', it.description || '') })) : [];
          return (fromSec.length > 0 ? fromSec : (toolsItems || []));
        }
        case 'prompts': return promptsItems || [];
        case 'learning': {
          const sec = (aiSections || []).find(s => s.id === 'learning');
          const fromSec = sec ? sec.items.map(it => ({ id: it.id, title: it.title, description: it.description || '', link: it.link || it.url || '#', image: (it.link || it.url) ? getFaviconUrl(it.link || it.url) : undefined, category: it.category || inferCategoryFromTitle(it.title || '', it.description || '') })) : [];
          return (fromSec.length > 0 ? fromSec : (learningItems || []));
        }
        case 'courses': {
          const sec = (aiSections || []).find(s => s.id === 'courses');
          const fromSec = sec ? sec.items.map(it => ({ id: it.id, title: it.title, description: it.description || '', link: it.link || it.url || '#', image: (it.link || it.url) ? getFaviconUrl(it.link || it.url) : undefined, category: it.category || inferCategoryFromTitle(it.title || '', it.description || '') })) : [];
          return (fromSec.length > 0 ? fromSec : (coursesItems || []));
        }
        case 'deeplearning': {
          const sec = (aiSections || []).find(s => s.id === 'deeplearning');
          const fromSec = sec ? sec.items.map(it => ({ id: it.id, title: it.title, description: it.description || '', link: it.link || it.url || '#', image: (it.link || it.url) ? getFaviconUrl(it.link || it.url) : undefined, category: it.category || inferCategoryFromTitle(it.title || '', it.description || '') })) : [];
          return (fromSec.length > 0 ? fromSec : (deeplearningItems || []));
        }
        case 'links': return linksItems || [];
        case 'tutorials': {
          const sec = (aiSections || []).find(s => s.id === 'tutorials');
          const fromSec = sec ? sec.items.map(it => ({ id: it.id, title: it.title, description: it.description || '', link: it.link || it.url || '#', image: (it.link || it.url) ? getFaviconUrl(it.link || it.url) : undefined, category: it.category || inferCategoryFromTitle(it.title || '', it.description || '') })) : [];
          // 合并教程、Markdown教程和博客文章
          const combinedTutorials = [...fromSec, ...tutorialsItems, ...markdownTutorials, ...blogPosts];
          // 根据日期排序
    ...
    const renderDetailContent = () => {
        ...
        // 生成详细的Markdown内容
        const markdownContent = `${selectedItem.content || selectedItem.description}`;
        ...
        try {
            ...
            if (activeTab.match(/^(general|painting|coding|voice|video|modeling|security|other|agent)$/)) {
                ...
                const relatedTutorials = [...tutorialsItems, ...markdownTutorials, ...blogPosts].filter(item => {
                    ...
                    return false; // To simplify, you can add more keyword matching logic here if needed.
                });
            ...
            if (activeTab === 'links') {
                ...
                // 优先从 content/ai/links.md 生成的数据中读取
                const linksSection = (aiSections || []).find((s) => s.id === 'links');
                const itemsFromSections: CardItem[] = linksSection
                ? linksSection.items.map((it) => ({
                    id: it.id,
                    title: it.title,
                ...
                : [];
                const itemsToRender = itemsFromSections.length > 0 ? itemsFromSections : (linksItems || []);
            ...
            } else if (activeTab === 'news' || activeTab === 'prompts' || activeTab === 'tutorials' || activeTab === 'courses' || activeTab === 'deeplearning' || activeTab === 'learning' ) {
                // 通用标签页渲染逻辑 - 使用卡片网格
