...
![](https://raw.githubusercontent.com/yy0691/img-bed/main/Net_Safe_FeiShu/B1mPbYuKZovz6wxODKtcg7oFnf4.png)
...
在每个<u><mark style={{"backgroundColor": "#F9D8B1"}}>主体上都附加</mark></u>一个该主体可访问的客体的明细表，根据表中信息的不同又可分成三种形式，即<mark style={{"backgroundColor": "#D4C1F3"}}>能力表（capabilitylist）、前缀表（profiles）和口令</mark>（password）
...
基于列的自主访问控制机制是在每个<u><mark style={{"backgroundColor": "#F9D8B1"}}>客体上都附加</mark></u>一个可访问它的主体的明细表，它有两种形式，即<mark style={{"backgroundColor": "#D4C1F3"}}>保护位（protectionbits）和访问控制表（AccessControlList，ACL）。</mark>
